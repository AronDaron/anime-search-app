...
- `ai.ts`: Plik łączący się z potężnym LLMem przez OpenRouter nakazujący na wyjściu odpowiedź `array` (dla wyszukiwania) lub `json_object` (dla opinii) w JSON. Obsługuje inteligentne ekstrahowanie intencji wyszukiwania (wzbogacone o wbudowaną bazę ~150 popularnych tagów Steam dla Gier) oraz funkcje syntezy recenzji. Wprowadzono funkcję `fetchAIRerankedGames`, która służy jako "Reranker" – analizuje pulę kandydatów ze Steama i wybiera te najlepiej pasujące do opisu użytkownika. Preferowany darmowy model: `google/gemini-2.5-flash-free`.
