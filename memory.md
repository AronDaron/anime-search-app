...
- `steamAPI.ts`, `steamReviews.ts` & `steamStore.ts` : Agregatory danych Steam. `steamAPI` obsługuje logikę proxy, w tym nowy tunel dla `api.steampowered.com`. `steamStore.ts` wzbogacono o funkcję `getSteamRealtimeCCU`, zapewniającą precyzyjne dane o liczbie graczy online prosto z serwerów Steam (identycznie jak na SteamDB).
...
- `GameDetails.tsx` / `GameDetails.css`: Zaawansowany interfejs wizytówki gry (profesjonalny styl Bento). Architektura oparta na **tucked grid layout**: 3-kolumnowy górny pas (**Powiększony do 320px Sidebar statystyk** | Centrum mediów z **2-rzędową galerią miniatur** | Buy Box). Statystyki ocen i graczy online zostały ujednolicone (użycie oficjalnego Steam API dla CCU oraz Steam Store API dla ocen). Interfejs został wyszlifowany: "Score Rank" wyświetla teraz minimalistyczny procent z polskim opisem w jednej linii, usunięto redundantne dane z Buy Boxa, a przycisk powrotu jest w pełni transparentny. Sekcja dolna (Wymagania i Werdykt AI) zintegrowana na pełną szerokość.
