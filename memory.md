...
- `GameDetails.tsx` / `GameDetails.css`: Zaawansowany interfejs wizytówki gry (profesjonalny styl Bento). Architektura oparta na **tucked grid layout**: 3-kolumnowy górny pas (**Powiększony do 320px Sidebar statystyk** | Centrum mediów z **2-rzędową galerią miniatur** | Buy Box). Sidebar statystyk (SteamSpy) wyświetla CCU, czas gry, oceny, sprzedaż oraz **Producenta/Wydawcę** i **Score Rank**. Sekcja dolna (Wymagania i Werdykt AI) została zintegrowana z gridem na **pełną szerokość (wszystkie 3 kolumny)**. Wdrożono zaawansowane stylowanie: kolorowe obramowania i ikonografię dla wymagań (Żółty/Zielony) oraz raportów AI (Zielony/Czerwony) dla maksymalnej czytelności w stylu premium.
