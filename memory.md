...
- `steamAPI.ts`, `steamReviews.ts` & `steamStore.ts` : Agregatory danych łączące się ze sklepem Steam i bazą SteamSpy. `steamAPI` rozpoznaje środowisko i dynamicznie ukierunkowuje requesty. Moduł `steamStore.ts` odpytuje o pełne detale gier, listę kategorii czy wyniki wyszukiwania. Został zoptymalizowany pod kątem **Deep Fetch**, pozwalając na pobieranie kompletnych list gier z gatunków (bez sztucznych limitów) oraz wykorzystuje nowoczesne CDNy Steam (`shared.akamai.steamstatic.com`).
...
- `GameFilterSearchView.tsx`: Zaawansowany panel wyszukiwania gier. Udostępnia w pełni reaktywne wyszukiwanie tekstowe z _debouncingiem_ w połączeniu z limitami cen Steam (z konwersją USD->PLN) i selektorami gatunków. Implementuje **Infinite Scroll** (widok paczkowany), który renderuje gry w partiach przy użyciu `IntersectionObserver`, co pozwala na płynne przeglądanie tysięcy wyników bez obciążania przeglądarki.
- `GameGenresView.tsx`: Dedykowana strona z asynchronicznie ładowaną siatką dla gier po wybranym gatunku. Posiada interfejs oparty na pasku neonowych tagów i również wykorzystuje mechanizm **Infinite Scroll**, aby zapewnić wydajność przy dużych wolumenach danych.
