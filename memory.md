...
- `steamAPI.ts`, `steamReviews.ts` & `steamStore.ts` : Agregatory danych łączące się ze sklepem Steam i bazą SteamSpy. `steamAPI` rozpoznaje środowisko i dynamicznie ukierunkowuje requesty. Moduł `steamStore.ts` został wzbogacony o inteligentną logikę hybrydową pod kątem **Deep Fetch** oraz funkcję `getSteamSpyTop100`, co pozwala na stabilne pobieranie kompletnych list bestsellerów i gatunków (bez limitów) przy użyciu nowoczesnych CDNów Steam (`shared.akamai.steamstatic.com`).
...
- `GameDetails.tsx` / `GameDetails.css`: Zaawansowany interfejs wizytówki gry (profesjonalny styl Bento). Architektura oparta na **tucked grid layout**: 3-kolumnowy górny pas (**Powiększony do 320px Sidebar statystyk** | Centrum mediów z **2-rzędową galerią miniatur** | Buy Box). Statystyki ocen zostały ujednolicone (użycie Steam Store API zamiast SteamSpy dla lepszej spójności), a pole "Score Rank" zastąpiono czytelnym zestawieniem procentowym wraz z polskim opisem tekstowym (np. "Bardzo pozytywne"). Usunięto redundantne bloki ocen z Buy Boxa dla czystszego układu. Sekcja dolna (Wymagania i Werdykt AI) zintegrowana na pełną szerokość. Wdrożono systemowe ikony i kolory dla statusów technicznych oraz analizy AI.
