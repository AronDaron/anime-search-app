        ...
        <nav className="navbar glass-panel">
            <div className="navbar-container">
                ...
                <div className="search-container" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div className={`search-bar ${isAiMode ? 'ai-active' : ''}`} style={isAiMode ? { borderColor: '#bf00ff', boxShadow: '0 0 10px rgba(191,0,255,0.3)' } : {}}>
                        {isAiMode ? <Sparkles className="search-icon" size={20} color="#bf00ff" /> : <SearchIcon className="search-icon" size={20} />}
                        <input
                            type="text"
                            className="search-input"
                            placeholder={isAiMode ? "Opisz fabułę... (np. magowie pocztowi)" : "Szukaj anime... (np. Jujutsu Kaisen)"}
                            value={searchQuery}
                    ...
                    <button className={`nav-btn ${path === '/anime/seasons' ? 'active' : ''}`} onClick={() => navigate('/anime/seasons')}>
                        Sezony
                    </button>
                </div>
