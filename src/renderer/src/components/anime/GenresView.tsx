    ...
    <div className="view-container fade-in">
      <div className="view-header">
        <h2 className="view-title">
          Gatunki <span className="neon-text-cyan">Anime</span>
        </h2>
      </div>
      ...
      >
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <select
            ...
            onChange={(e) =>
              setSelectedYear(
                e.target.value === 'Wszystkie' ? 'Wszystkie' : parseInt(e.target.value)
              )
            }
            style={{
          ...
          >
            <option value="Wszystkie" style={{ background: '#111' }}>
              Wszystkie lata
            </option>
            {YEARS.map((year) => (
        ...
        <>
          {results.length === 0 ? (
            <div className="empty-state">
              <p>
                Nie odnaleziono anime dla gatunku "
                {GENRES.find((g) => g.id === selectedGenre)?.label || selectedGenre}"
                {selectedYear !== 'Wszystkie' ? ` w roku ${selectedYear}` : ''}.
              </p>
            </div>
          ) : (
