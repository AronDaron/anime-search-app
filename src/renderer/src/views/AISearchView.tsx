...
export const AISearchView: React.FC = () => {
    ...
    const handleSearch = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            console.log("Wysyłanie zapytania do AI:", fullPrompt);
            const aiResult = await fetchAIAnimeTitles(fullPrompt, apiKey);
            ...
            if (aiResult.searchParams) {
                ...
                if ((aiResult.searchParams.genres && aiResult.searchParams.genres.length > 0) ||
                    (aiResult.searchParams.tags && aiResult.searchParams.tags.length > 0) ||
                    yearToUse || seasonToUse) {
                    ...
                    try {
                        ...
                        if (advancedSearchResults.Page.media.length > 0) {
                            ...
                            const newUnique = advancedSearchResults.Page.media.filter(a => !existingIds.has(a.id));
            ...
            if (combinedResults.length === 0) {
                setError("Nie znaleziono żadnych pasujących anime. Spróbuj sformułować opis inaczej lub zmniejszyć liczbę filtrów.");
            } else {
        ...
        <div className="ai-search-container fade-in">
            <div className="ai-header">
                ...
                <p className="ai-subtitle">
                    Opisz anime, którego szukasz (np. <span className="highlight-pink">"Wampiry i akcja z lat 90"</span> lub <span className="highlight-cyan">"Nowość o magii ze świetną animacją"</span>). Sztuczna inteligencja przeanalizuje Twoje zapytanie i przeszuka bazę danych.
                </p>
            </div>
                ...
                <div className="ai-search-inner">
                    <form onSubmit={handleSearch}>
                        <div className="ai-textarea-wrapper">
                            ...
                            />
                            <div className="ai-textarea-hint">
                                Naciśnij Enter, aby szukać
                            </div>
                        </div>
                                ...
                                >
                                    <div className="ai-filters-panel">
                                        <div className="ai-filter-group">
                                            ...
                                            />
                                            <p className="ai-filter-hint">AI zignoruje rok z tekstu, jeśli to wypełnisz.</p>
                                        </div>
                                        <div className="ai-filter-group">
                    ...
                    <div className="ai-results-header">
                        <h2 className="ai-results-title">
                            ...
                            <span className="ai-results-count">
                                {results.length} tytułów
                            </span>
                        </h2>

                        {aiParams && (
                            <div className="ai-tags-list">
                                {aiParams.genres && aiParams.genres.map(g => (
                                    <span key={g} className="ai-tag">
                                        {g}
                                    </span>
                                ))}
                                {aiParams.seasonYear && (
                                    <span className="ai-tag year">
                                        Rocznik: {aiParams.seasonYear}
                                    </span>
                                )}
                            </div>
                        )}
